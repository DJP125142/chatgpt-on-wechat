...
class TimeTaskModel:
    ...
    # 指定时区
    def nowTime(self):
        return arrow.now().to(self.timezone)
    ...
    def get_todayCron_times(self):
        ...
        if self.isValid_Cron_time():
            # 获取当前时间（忽略秒数）
            current_time = self.nowTime().replace(second=0, microsecond=0)
            # 创建一个 croniter 对象
    ...
    def is_featureTime(self):
        ...
        if self.isCron_time():
            ...
            #对比精准到分（忽略秒）
            current_time = self.nowTime().replace(second=0, microsecond=0).time()
    ...
    def is_featureDay(self):
        ...
        if self.isCron_time():
            ...
            if self.is_valid_date(tempStr):
                tempValue = arrow.get(tempStr, 'YYYY-MM-DD').date() > self.nowTime().date()
